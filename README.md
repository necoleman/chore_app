...
- `sort_last` — `TRUE` sinks the chore to the **end of its cadence group** on Today, for work that can only be done later in the day (e.g. after-dinner chores). Flagged cards carry a small clock icon. Leave blank for everything else. Set it via the chore editor's "Show at the end of its group" checkbox.
