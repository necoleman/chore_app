...
- `on_vacation` — `TRUE` while a person is away (#29). Toggle it from Manage Chores → People ("Set vacation" / "End vacation"). While set, their currently-open assignments are moved to unclaimed and rotations skip them (a rotation with other available people still generates to the next person). A chore whose default assignee(s) are **all** on vacation is **paused** (#34): the generator advances its schedule anchor but creates no assignment and accrues no `missed_count` or points penalty, so nothing piles up mid-vacation. A chore that is sent back (rejected) while its owner is away is also moved to unclaimed, so it doesn't sit stranded on their list aging all trip. Ending vacation re-homes the chores whose **sole** default assignee is that person back to them; an occurrence that went past due while they were gone is **re-dated to today with its miss count cleared** (nobody could have done it), while one still due today or later keeps its original date. Managed by the app — leave blank otherwise.
