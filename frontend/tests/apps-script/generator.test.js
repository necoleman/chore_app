...
describe('cadence helpers after the custom merge (#45)', () => {
  const { ctx } = loadBackend();
  it('recurrencePeriodDays no longer knows about custom', () => {
    expect(ctx.recurrencePeriodDays({ frequency: 'weekly' })).toBe(7);
    expect(ctx.recurrencePeriodDays({ frequency: 'monthly' })).toBe(28);
    expect(ctx.recurrencePeriodDays({ frequency: 'interval', interval_days: '90' })).toBe(90);
    // Daily is 1 whether or not weekday_due pins it, which is what keeps its
    // lead capped at 1 — the strictness the custom merge was for.
    expect(ctx.recurrencePeriodDays({ frequency: 'daily' })).toBe(1);
    expect(ctx.recurrencePeriodDays({ frequency: 'daily', weekday_due: '1,4' })).toBe(1);
  });
});
